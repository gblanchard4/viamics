...
def _exec(p, request_dict):
    ...
    blast_db = os.path.join(c.blast_db_dir,request_dict['db_name'])
    
    debug(("running blast on %d sequences against database: %s " % (number_of_sequences, request_dict['db_name'])), p.files.log_file)
...
def samples_dictionary(p):
    ...
    legend_path = os.path.join(c.blastdb_dir,
                               open(p.blast_db_name_path).read(),
                               c.blast_legend_file_extension)
    samples_dict = framework.tools.blast.create_samples_dictionary(p.files.data_file_path,
                                                                   legend_path,
...
def otu_library(p):
    ...
    otu_library = framework.tools.blast.get_otu_library(p.files.data_file_path,
                                                        legend_path,
                                                        open(p.files.seperator_file_path).read())
